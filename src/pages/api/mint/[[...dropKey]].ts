...
export default async function handler(
  ...
  try {
    ...
    const { event }: { event?: "bh" | "recife" | "vitoria" } = req.query;
    ...
    const nftToMint = STATIC_NFT_ITEMS.bh;
    ...
    if (req.method != "post") {
      return res.status(200).json(
        Object.assign(
          buildSolanaPayGetResponse({
            message: STATIC_NFT_ITEMS.bh.metadata.name,
            label: SITE_NAME,
          }),
    ...
    return res.status(200).json(
      buildSolanaPayPostResponse({
        message: STATIC_NFT_ITEMS.bh.metadata.name, // drop.metadata.name - should we show the nft name?,
        transaction,
        signers: [SOLANA_KEYPAIR],
      }),
    );
